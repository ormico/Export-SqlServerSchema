...
function Invoke-ScriptForOutput {
    ...
    $wrapperTempDir = if ($env:RUNNER_TEMP) { $env:RUNNER_TEMP } else { [System.IO.Path]::GetTempPath() }
    $wrapperPs1 = Join-Path $wrapperTempDir ([System.IO.Path]::GetRandomFileName() + '.ps1')
    @"
