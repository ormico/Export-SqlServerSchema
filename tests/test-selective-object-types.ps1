...
# Load configuration from .env file (same pattern as run-integration-test.ps1)
$envFile = Join-Path $PSScriptRoot $ConfigFile
...
# Build credential from .env values (SA_PASSWORD variable)
$Credential = $null
if ($script:SA_PASSWORD) {
    $securePassword = ConvertTo-SecureString $script:SA_PASSWORD -AsPlainText -Force
