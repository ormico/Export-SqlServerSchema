...
<#
.SYNOPSIS
    Tests the -ExcludeSchemas parameter for Import-SqlServerSchema.ps1

.DESCRIPTION
    This test validates that the -ExcludeSchemas parameter correctly filters out
    scripts by schema name during import. It tests:
    1. Command-line parameter exclusion
...
# Fixed database names - prevents SQL injection issues
$SourceDatabase = 'TestDb_SchemaSource'
$TargetDatabase = 'TestDb_SchemaExclude'
...
# Build sqlcmd arguments as array (avoids string-splitting issues with spaces)
$sqlcmdArgs = @('-S', $Server, '-U', 'sa', '-P', $Password, '-C')
...
# Initialize variables used in cleanup (so they exist even if script fails early)
$configFile = $null
...
IF EXISTS (SELECT 1 FROM sys.databases WHERE name = 'TestDb_SchemaExclude')
    ALTER DATABASE [TestDb_SchemaExclude] SET SINGLE_USER WITH ROLLBACK IMMEDIATE;
IF EXISTS (SELECT 1 FROM sys.databases WHERE name = 'TestDb_SchemaExclude')
...
"@
    ...
    # Check that fewer scripts were executed (16 total - 3 cdc = 13)
    # The actual number of scripts depends on export but should be less than total
    $scriptCountMatch = [bool]($importResult | Where-Object { $_ -match 'Found\s+(\d+)\s+script' })
    Write-TestResult -Name "Import executed scripts (count check)" -Passed $scriptCountMatch

    # Verify cdc objects were NOT created
    $checkCdcSql = "USE [TestDb_SchemaExclude]; SELECT COUNT(*) FROM sys.objects WHERE SCHEMA_NAME(schema_id) = 'cdc'"
    $cdcObjectCount = Get-SqlScalarValue (sqlcmd @sqlcmdArgs -Q $checkCdcSql -h -1 2>&1)
    ...
    $checkAppSql = "USE [TestDb_SchemaExclude]; SELECT COUNT(*) FROM sys.objects WHERE SCHEMA_NAME(schema_id) = 'app'"
    $appObjectCount = Get-SqlScalarValue (sqlcmd @sqlcmdArgs -Q $checkAppSql -h -1 2>&1)
    ...
    $checkDboSql = "USE [TestDb_SchemaExclude]; SELECT COUNT(*) FROM sys.objects WHERE SCHEMA_NAME(schema_id) = 'dbo' AND type IN ('U','P','FN')"
    $dboObjectCount = Get-SqlScalarValue (sqlcmd @sqlcmdArgs -Q $checkDboSql -h -1 2>&1)
...
"@
    ...
    $cdcObjectCount2 = Get-SqlScalarValue (sqlcmd @sqlcmdArgs -Q $checkCdcSql -h -1 2>&1)
    ...
    $checkStagingSql = "USE [TestDb_SchemaExclude]; SELECT COUNT(*) FROM sys.objects WHERE SCHEMA_NAME(schema_id) = 'staging'"
    $stagingObjectCount = Get-SqlScalarValue (sqlcmd @sqlcmdArgs -Q $checkStagingSql -h -1 2>&1)
    ...
    $appObjectCount2 = Get-SqlScalarValue (sqlcmd @sqlcmdArgs -Q $checkAppSql -h -1 2>&1)
    ...
    $cdcObjectCount3 = Get-SqlScalarValue (sqlcmd @sqlcmdArgs -Q $checkCdcSql -h -1 2>&1)
    ...
    $stagingObjectCount3 = Get-SqlScalarValue (sqlcmd @sqlcmdArgs -Q $checkStagingSql -h -1 2>&1)
    ...
    $appObjectCount3 = Get-SqlScalarValue (sqlcmd @sqlcmdArgs -Q $checkAppSql -h -1 2>&1)
...
"@
    ...
    if ($configFile -and (Test-Path $configFile)) { Remove-Item $configFile -Force }
