...
$script:ParallelWorkerScriptBlock = {
  ...
  while ($true) {
    ...
    try {
      ...
      # RoleMembers: write raw SQL directly (no SMO Scripter needed)
      if ($workItem.SpecialHandler -eq 'RoleMembers' -and $workItem.CustomData -and $workItem.CustomData.Statements) {
        $content = "-- Database Role Memberships`r`n-- Generated by Export-SqlServerSchema`r`n`r`n"
        $content += ($workItem.CustomData.Statements -join "`r`nGO`r`n")
        $content += "`r`nGO`r`n"
        [System.IO.File]::WriteAllText($workItem.OutputPath, $content, [System.Text.UTF8Encoding]::new($true))
        $result.Success = $true
      }
      else {
      ...
      $result.Success = $true
      } # end else (non-RoleMembers SMO scripting path)
    }
    catch {
...
function Build-WorkItems-Security {
  ...
  # Role Members - exports ALTER ROLE ... ADD MEMBER statements for all role memberships
  # Must come after both Roles (006) and Users (008) since it references both
  if (-not (Test-ObjectTypeExcluded -ObjectType 'RoleMembers')) {
    try {
      $roleMemberStatements = [System.Collections.Generic.List[string]]::new()
      ...
      foreach ($role in $allRoles) {
        try {
          ...
          Write-Verbose "Could not enumerate members for role '$($role.Name)': $_"
        }
      }

      # Also enumerate members of fixed roles (db_owner, db_datareader, etc.)
      # These memberships are critical for user permissions but the roles themselves are not scripted
      $fixedRoles = @($Database.Roles | Where-Object { $_.IsFixedRole -and $_.Name -ne 'public' })
      foreach ($role in $fixedRoles) {
        try {
          ...
          Write-Verbose "Could not enumerate members for fixed role '$($role.Name)': $_"
        }
      }

      if ($roleMemberStatements.Count -gt 0) {
        $workItems += New-ExportWorkItem `
          -ObjectType 'RoleMember' `
          -GroupingMode 'all' `
          -Objects @(@{ Schema = $null; Name = 'RoleMembers' }) `
          -OutputPath (Join-Path $OutputDir '01_Security_RoleMembers' 'RoleMembers.sql') `
          -ScriptingOptions @{} `
          -SpecialHandler 'RoleMembers' `
          -CustomData @{ Statements = $roleMemberStatements.ToArray() }
      }
    }
    catch { Write-Verbose "Could not enumerate role memberships: $_" }
  }
...
function Save-ExportMetadata {
  ...
  $folderTypeMap = @{
    ...
    '01_Security_RoleMembers'  = 'RoleMember'
...
function Export-DatabaseObjects {
  ...
  if ($workItems.Count -eq 0) {
    ...
    $objectTypeOrder = @(
      ...
      'RoleMember', 'DatabaseAuditSpecification', 'SecurityPolicy'
    )
...
function New-DeploymentManifest {
  ...
  [void]$sb.AppendLine("7. 06_PartitionSchemes - Create partition schemes")
  [void]$sb.AppendLine("8. 07_Types - Create user-defined types")
  [void]$sb.AppendLine("9. 08_XmlSchemaCollections - Create XML schema collections")
  [void]$sb.AppendLine("10. 09_Tables_PrimaryKey - Create tables with primary keys (no foreign keys)")
  [void]$sb.AppendLine("11. 10_Indexes - Create indexes")
  [void]$sb.AppendLine("12. 11_Tables_ForeignKeys - Add foreign key constraints")
  [void]$sb.AppendLine("13. 12_Defaults - Create default constraints")
  [void]$sb.AppendLine("14. 13_Rules - Create rules")
  [void]$sb.AppendLine("15. 14_Programmability - Create assemblies, functions, procedures, triggers, views (in subfolder order)")
  [void]$sb.AppendLine("16. 15_Synonyms - Create synonyms")
  [void]$sb.AppendLine("17. 16_FullTextSearch - Create full-text search objects")
  [void]$sb.AppendLine("18. 17_ExternalData - Create external data sources and file formats (review connection strings)")
  [void]$sb.AppendLine("19. 18_SearchPropertyLists - Create search property lists")
  [void]$sb.AppendLine("20. 19_PlanGuides - Create plan guides")
  [void]$sb.AppendLine("21. 20_SecurityPolicies - Create Row-Level Security policies (requires schemas and predicate functions)")
  [void]$sb.AppendLine("22. 21_Data - Load data")
  [void]$sb.AppendLine("")
